...
class CifarClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        metrics = self.train(epochs=2, device=DEVICE)
        return self.get_parameters(config={}), 60, metrics.results_dict

    def evaluate(self, parameters, config):
        self.set_parameters(parameters)
        metrics = self.test(device=DEVICE)
        return 1 - metrics.results_dict['metrics/mAP50-95(B)'], 60, metrics.results_dict

    def train(self, epochs, device):
        """Train the network on the training set."""
        return self.net.train(data=DATA_YAML, epochs=epochs, imgsz=640, device=device)
